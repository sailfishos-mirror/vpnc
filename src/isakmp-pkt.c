...
struct isakmp_payload *new_isakmp_data_payload(uint8_t type, const void *data, size_t data_length)
{
	...
	if (type != ISAKMP_PAYLOAD_KE && type != ISAKMP_PAYLOAD_HASH && type != ISAKMP_PAYLOAD_SIG && type != ISAKMP_PAYLOAD_NONCE && type != ISAKMP_PAYLOAD_VID && type != ISAKMP_PAYLOAD_NAT_D && type != ISAKMP_PAYLOAD_NAT_D_OLD)
		abort();
...
#define fetch4()                   \
	(data += 4, data_len -= 4, \
	 (uint32_t)(data[-4]) << 24 | (uint32_t)(data[-3]) << 16 | (uint32_t)(data[-2]) << 8 | data[-1])
#define fetch2()                   \
	(data += 2, data_len -= 2, \
							...
							size_t data_len, int *reject, enum isakmp_ipsec_proto_enum decode_proto)
{
	...
	if (type & 0x8000) {
		...
		r->af = isakmp_attr_16;
		r->u.attr_16 = length;
		if ((ISAKMP_XAUTH_06_ATTRIB_TYPE <= r->type) && (r->type <= ISAKMP_XAUTH_06_ATTRIB_ANSWER) && (r->type != ISAKMP_XAUTH_06_ATTRIB_STATUS) && (length > 0) && (opt_debug < 99))
		...
		if (r->type == ISAKMP_MODECFG_ATTRIB_CISCO_SPLIT_INC) {
			...
			if ((ISAKMP_XAUTH_06_ATTRIB_TYPE <= type) && (type <= ISAKMP_XAUTH_06_ATTRIB_ANSWER) && (r->type != ISAKMP_XAUTH_06_ATTRIB_STATUS) && (length > 0) && (opt_debug < 99))
				DEBUG(3, printf("(not dumping xauth data)\n"));
			else
						   ...
						   const uint8_t **data_p, size_t *data_len_p, int *reject, enum isakmp_ipsec_proto_enum decode_proto)
{
	...
	if (length > data_len + 4 || ((type <= ISAKMP_PAYLOAD_MODECFG_ATTR) && (length < min_payload_len[type])) || (length < 4)) {
		*reject = ISAKMP_N_PAYLOAD_MALFORMED;
		return r;
	}
	olength = length;
...
void test_pack_unpack(void)
{
	...
	if (unpack_len != sizeof(pack) || memcmp(unpack, pack, sizeof(pack)) != 0)
		abort();
	free(unpack);
